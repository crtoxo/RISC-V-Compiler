...
SymbolEntry::SymbolEntry(const std::string &p_name,
                         ...
                         const int loc_offset
                        )
    : name(p_name), kind(kind), level(level), type(p_type),
      attribute(p_constant), prev_entry(p_prev), has_error(false), LocationOffset(loc_offset) {}
...
//void SymbolEntry::setLocationOffset(int i) const { LocationOffset = i; }

int SymbolEntry::getLocationOffset() const { return LocationOffset; }
...
SymbolEntry *SymbolTable::addSymbol(const std::string &p_name,
    ...
    entries.emplace_back(
        new SymbolEntry(p_name, kind, level, p_type, p_constant, p_prev, loc_offset));
...
SymbolEntry *SymbolManager::addSymbol(const std::string &p_name,
    ...
    auto *new_entry = current_table->addSymbol(
        p_name, kind, current_level, p_type, p_constant, result_pair.second, current_table->getAndSetLocationOffset());
