...
class SymbolEntry {
  public:
    ...
    //void setLocationOffset(int i) const;
    int getLocationOffset() const;
...
class SymbolTable {
  public:
    SymbolTable() { location_offset = -12; }
    ~SymbolTable() = default;
    ...
    SymbolEntry *addSymbol(const std::string &p_name,
                           ...
                           const SymbolEntry *p_prev, const int loc_offset);
    SymbolEntry *addSymbol(const std::string &p_name,
                           const SymbolEntry::KindEnum kind, const size_t level,
                           const PType *p_type,
                           const FunctionNode::Decls *p_parameters,
                           const SymbolEntry *p_prev);
    int getAndSetLocationOffset() { location_offset -= 4;  return location_offset + 4; }
